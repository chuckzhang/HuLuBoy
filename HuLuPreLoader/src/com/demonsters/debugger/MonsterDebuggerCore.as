...
package com.demonsters.debugger
{
	...
	internal class MonsterDebuggerCore
	{
		...
		private static function set highlightTarget(value:DisplayObject):void
		{
			if(_highlightTarget&&_highlightTarget.hasEventListener(MouseEvent.MOUSE_DOWN))
			{
				_highlightTarget.removeEventListener(MouseEvent.MOUSE_DOWN,onMouseDown);
			}
		...
		{	
			...
			_base.stage.addEventListener(MouseEvent.MOUSE_UP,onMouseUp);
		...
		private static function highlightClicked(event:MouseEvent):void
		{
			...
			highlightTarget = MonsterDebuggerUtils.getObjectUnderPoint(_stage, new Point(_stage.mouseX, _stage.mouseY));
			
			// Stop mouse interactions
			_highlightMouse = false;
			_highlight.removeEventListener(MouseEvent.CLICK, highlightClicked);
			_highlight.mouseEnabled = false;
			_highlightTarget.addEventListener(MouseEvent.MOUSE_DOWN,onMouseDown);
			_base.stage.addEventListener(KeyboardEvent.KEY_UP,onCancelSelect);
			// Inspect
		...
		protected static function onCancelSelect(event:KeyboardEvent):void
		{
		...
		protected static function onMouseUp(event:Event):void
		{
			_base.stage.removeEventListener(MouseEvent.MOUSE_MOVE,onMouseMove);			
		}
		...
		{
			_base.stage.addEventListener(MouseEvent.MOUSE_MOVE,onMouseMove);	
			lastX=event.stageX;
			lastY=event.stageY;
		}
		...
		{
			_highlightTarget.x+=(event.stageX-lastX);
			_highlightTarget.y+=(event.stageY-lastY);
			lastX=event.stageX;
			lastY=event.stageY;
			highlightUpdate();
